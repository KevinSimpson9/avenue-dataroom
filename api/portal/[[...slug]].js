...
async function postResendLink(req, res) {
  ...
  if (kind === 'ready-to-sign' || kind === 'investor-signature') {
    if (!entry.lukasSignedAt) return res.status(409).json({ ok: false, message: 'Lukas has not signed this note yet.' });
    if (entry.signedAt) return res.status(409).json({ ok: false, message: 'Investor has already signed.' });
    let setupToken = null;
    if (!entry.passwordHash) setupToken = issueToken({ email: entry.email, purpose: 'setup' });
  ...
  if (kind === 'lukas-signature') {
    if (entry.lukasSignedAt) return res.status(409).json({ ok: false, message: 'Lukas has already signed this note.' });
